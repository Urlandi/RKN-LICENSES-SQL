...
from data_plot import plot_licenses

DONE = 0
STOP_AND_EXIT = 1

XML_NAME = "data-20240304T0000-structure-20220708T0000.xml"
...
def main():
    ...
    if db is not None and licenses is not None and False:
        for record in licenses:
            ...
            if lic_record['registration'] is not None:
                registration = add_order(db, lic_record['registration']['num'],
                                         int(datetime.strptime(
                                             lic_record['registration']['date'], '%Y-%m-%d').
            ...
            if lic_record['date_start'] is not None:
                date_start = int(datetime.strptime(lic_record['date_start'], '%Y-%m-%d').
                                 replace(tzinfo=timezone.utc).timestamp())

            date_end = None
            if lic_record['date_end'] is not None:
                date_end = int(datetime.strptime(lic_record['date_end'], '%Y-%m-%d').
                               replace(tzinfo=timezone.utc).timestamp())

            date_service_start = None
            if lic_record['date_service_start'] is not None:
                date_service_start = int(datetime.strptime(lic_record['date_service_start'], '%Y-%m-%d').
                                         replace(tzinfo=timezone.utc).timestamp())
